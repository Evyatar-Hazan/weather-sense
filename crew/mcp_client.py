...
class MCPClient:
    def __init__(self, timeout: int = 30):
        ...
        self.mcp_script_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "mcp_weather", "server.py"
        )
    ...
    def _call_persistent_process(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                _persistent_mcp_process is None
                or _persistent_mcp_process.poll() is not None
            ):
    ...
    def _call_subprocess(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if proc.returncode != 0:
                logger.error(
                    f"MCP process failed with code {proc.returncode}: {stderr}"
                )
                return {
...
def start_persistent_mcp_server() -> bool:
    ...
    try:
        ...
        if _persistent_mcp_process.poll() is None:
            logger.info(
                f"Persistent MCP server started with PID {_persistent_mcp_process.pid}"
            )
            return True
        else:
