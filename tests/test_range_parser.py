...
class TestRangeParserHappyPath:
    ...
    @pytest.mark.parametrize(
        "query,expected_location,expected_units",
        [
            (
                "weather in Tel Aviv from tomorrow to next Friday, metric",
                "Tel Aviv",
                "metric",
            ),
            (
                "temperature in New York from tomorrow to next week, imperial",
                "New York",
                "imperial",
            ),
            ...
            (
                "climate data for London from tomorrow to next Sunday",
                "London",
                "metric",
            ),
        ],
    )
    def test_successful_parsing_various_formats(
        self, query, expected_location, expected_units
    ):
        """
...
class TestRangeParserEdgeCases:
    ...
    def test_missing_location_produces_error(self):
        ...
        result = parse_natural_language(
            {"query": "weather forecast from tomorrow to next week"}
        )
    ...
    def test_missing_units_defaults_to_metric(self):
        ...
        """
        result = parse_natural_language(
            {"query": "weather in Berlin from tomorrow to next week"}
        )
...
class TestRangeParserRobustness:
    ...
    def test_multiple_locations_in_query(self):
        ...
        if "error" not in result:
            ...
            assert (
                result["location"] in ["London", "Paris"] or result["confidence"] < 0.8
            )
...
class TestRangeParserParametrized:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "climate in Cairo from tomorrow to next Sunday, metric",
                "Cairo",
                "metric",
            ),
    ...
    def test_comprehensive_query_variations(
        self, query, expected_location, expected_units
    ):
        """
