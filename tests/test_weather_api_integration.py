...
class TestWeatherAskAuth:
    ...
    def test_valid_api_key_returns_200_or_processing_error(self, client, valid_headers, sample_query):
...
class TestWeatherAskSuccess:
    ...
    def test_weather_ask_success_response_schema(self, client, valid_headers):
        ...
        if response.status_code != 200:
            pytest.skip(f"External dependency unavailable (status: {response.status_code})")
        
        data = response.json()
        
        # Validate all required fields are present
        required_fields = [
            "summary", "params", "data", "confidence", 
            "tool_used", "latency_ms", "request_id"
        ]
        ...
        assert not any(placeholder in data["summary"].lower() 
...
class TestWeatherAskPerformance:
    ...
    def test_latency_measurement_accuracy(self, client, valid_headers):
        ...
        tolerance = 0.10
        assert abs(reported_latency_ms - measured_duration_ms) / measured_duration_ms <= tolerance, \
    ...
    def test_request_includes_structured_logs(self, client, valid_headers, caplog):
        ...
        assert found_request_id, f"request_id {request_id} not found in logs. Available records: {[r.message for r in caplog.records]}"
