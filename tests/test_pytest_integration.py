...
class TestDeploymentDocumentation:
    ...
    def test_readme_cloud_run_section_exists(self, workspace_root):
        ...
        assert (
            "gcloud run deploy" in readme_content
        ), "gcloud run deploy command not found"
        assert (
            "--allow-unauthenticated" in readme_content
        ), "--allow-unauthenticated flag not documented"
