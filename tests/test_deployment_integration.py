    ...
    def start_test_container(self):
        ...
        try:
            ...
            self.docker_client.images.build(
                path=str(self.workspace_root), tag=self.test_image_tag, rm=True
            )
        ...
        try:
            ...
            assert (
                "API server started with PID" in logs
            ), f"API server not started. Logs: {logs}"
            assert (
                "All services started successfully" in logs
            ), f"Not all services started. Logs: {logs}"
...
class TestSingleDockerImage(DeploymentIntegrationTest):
    ...
    def test_mcp_communication_works(self):
        ...
        weather_query = {"query": "weather in Tel Aviv from 2025-10-20 to 2025-10-22"}

        headers = {"Content-Type": "application/json", "x-api-key": self.test_api_key}
        ...
        try:
            ...
            assert (
                "error" in result or "summary" in result
            ), "Response should contain error or summary"

            logger.info(
                "✅ MCP communication working - API and MCP server communicate correctly"
            )
...
class TestCloudRunDeployment(DeploymentIntegrationTest):
    ...
    def test_readme_contains_valid_gcloud_deploy_command(self):
        ...
        logger.info(
            "✅ README contains valid gcloud run deploy command with all required flags"
        )
    ...
    def test_dockerfile_exposes_correct_port(self):
        ...
        # Check port exposure
        assert (
            "EXPOSE ${PORT}" in dockerfile_content
            or "EXPOSE 8000" in dockerfile_content
        ), "Dockerfile should expose port 8000 or ${PORT}"

        # Check entrypoint
        assert (
            'CMD ["python", "docker_entrypoint.py"]' in dockerfile_content
        ), "Dockerfile should use docker_entrypoint.py as CMD"

        # Check environment variables
        assert (
            "ENV" in dockerfile_content
        ), "Dockerfile should set environment variables"
        assert "PORT=8000" in dockerfile_content, "Dockerfile should set PORT=8000"

        logger.info(
            "✅ Dockerfile has correct port exposure and entrypoint configuration"
        )
    ...
    def test_required_environment_variables_documented(self):
        ...
        for var in required_vars:
            ...
            ), f"Required environment variable '{var}' not documented in README"
        ...
        found_env_section = any(
            pattern in readme_content for pattern in env_section_patterns
        )
...
class TestAuthenticationBehavior(DeploymentIntegrationTest):
    ...
    def test_weather_endpoint_requires_api_key(self):
        ...
        try:
            response = requests.post(
                f"{self.api_base_url}/v1/weather/ask", json=weather_query, timeout=10
            )
...
class TestProcessManagement(DeploymentIntegrationTest):
    ...
    def test_docker_entrypoint_starts_api_server(self):
        ...
        assert (
            "All services started successfully" in logs
        ), "Services startup confirmation missing"
    ...
    def test_mcp_server_communication_via_stdio(self):
        ...
        try:
            ...
            if result.stdout:
                ...
                assert isinstance(
                    response, dict
                ), "MCP server should return JSON object"

            # MCP server should use stdio communication
            assert result.returncode in [
                0,
                1,
            ], f"MCP server returned unexpected code: {result.returncode}"
    ...
    def test_container_logs_show_both_services(self):
        ...
        api_log_indicators = ["API server started", "uvicorn", "FastAPI"]
        ...
        assert (
            "All services started successfully" in logs
        ), "Service startup confirmation missing"
...
class TestDocumentationValidation(DeploymentIntegrationTest):
    ...
    def test_readme_cloud_run_section_complete(self):
        ...
        assert (
            "Google Cloud Run" in readme_content
        ), "Cloud Run section missing from README"
        ...
        for section in deployment_sections:
            ...
            ), f"Deployment section '{section}' missing from README"

        # Check for environment variables documentation
        assert (
            "Environment Variables" in readme_content
        ), "Environment variables section missing"
    ...
    def test_dockerfile_cloud_run_optimized(self):
        ...
        for practice in cloud_run_practices:
            assert (
                practice in dockerfile_content
            ), f"Cloud Run best practice '{practice}' missing from Dockerfile"
