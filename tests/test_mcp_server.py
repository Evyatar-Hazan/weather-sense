...
class TestProcessWeatherRequestFlow:
    ...
    def test_successful_request_with_cache_hit(self, mock_provider_class, mock_cache):
        ...
        mock_provider = Mock()
        mock_provider_class.return_value = mock_provider
        mock_provider.geocode_location.return_value = (
            32.08088,
            34.78057,
            "Tel Aviv, Israel",
        )
    ...
    def test_geocoding_error_handling(self, mock_provider_class, mock_cache):
        ...
        mock_provider = Mock()
        mock_provider_class.return_value = mock_provider
        mock_provider.geocode_location.side_effect = ValueError(
            "Location 'NonexistentPlace123' not found"
        )
...
class TestPersistentMode:
    ...
    @patch("sys.stdout")
    @patch("server.process_weather_request")
    def test_persistent_mode_multiple_requests(
        self, mock_process, mock_stdout, mock_stdin
    ):
