...
class TestWeatherAnalystHappyFlow:
    ...
    def base_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "weather_raw": {
                ...
                "end_date": "2025-10-25",
                "daily": [
                    {"date": "2025-10-20", "tmin": 19.8, "tmax": 26.8, "precip_mm": 0.0, "wind_max_kph": 12.0, "code": 3},
                    {"date": "2025-10-21", "tmin": 18.5, "tmax": 25.2, "precip_mm": 2.5, "wind_max_kph": 15.2, "code": 61},
                    {"date": "2025-10-22", "tmin": 20.1, "tmax": 28.3, "precip_mm": 0.0, "wind_max_kph": 8.7, "code": 1},
                    {"date": "2025-10-23", "tmin": 17.2, "tmax": 24.9, "precip_mm": 8.2, "wind_max_kph": 22.1, "code": 63},
                    {"date": "2025-10-24", "tmin": 21.0, "tmax": 29.5, "precip_mm": 0.0, "wind_max_kph": 9.3, "code": 0},
                    {"date": "2025-10-25", "tmin": 19.6, "tmax": 27.1, "precip_mm": 0.3, "wind_max_kph": 11.8, "code": 2}
                ],
                "source": "open-meteo"
            }
        }
    ...
    def imperial_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "weather_raw": {
                ...
                "end_date": "2025-10-23",
                "daily": [
                    {"date": "2025-10-20", "tmin": 45.2, "tmax": 68.4, "precip_mm": 0.0, "wind_max_kph": 18.5, "code": 1},
                    {"date": "2025-10-21", "tmin": 42.1, "tmax": 65.7, "precip_mm": 12.7, "wind_max_kph": 28.3, "code": 65},
                    {"date": "2025-10-22", "tmin": 38.9, "tmax": 59.2, "precip_mm": 25.4, "wind_max_kph": 45.2, "code": 82},
                    {"date": "2025-10-23", "tmin": 48.6, "tmax": 72.1, "precip_mm": 0.0, "wind_max_kph": 12.1, "code": 0}
                ],
                "source": "open-meteo"
            }
        }
    ...
    def extreme_weather_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "weather_raw": {
                ...
                "end_date": "2025-10-24",
                "daily": [
                    {"date": "2025-10-20", "tmin": 2.1, "tmax": 8.7, "precip_mm": 0.0, "wind_max_kph": 15.2, "code": 71},  # Snow
                    {"date": "2025-10-21", "tmin": -1.5, "tmax": 5.2, "precip_mm": 15.8, "wind_max_kph": 52.3, "code": 95},  # Thunderstorm
                    {"date": "2025-10-22", "tmin": 18.9, "tmax": 32.1, "precip_mm": 0.0, "wind_max_kph": 8.1, "code": 0},   # Clear
                    {"date": "2025-10-23", "tmin": 12.4, "tmax": 25.7, "precip_mm": 22.3, "wind_max_kph": 41.7, "code": 65}, # Heavy rain
                    {"date": "2025-10-24", "tmin": 15.6, "tmax": 28.9, "precip_mm": 1.2, "wind_max_kph": 19.4, "code": 3}   # Overcast
                ],
                "source": "open-meteo"
            }
        }
    ...
    def test_full_analysis_flow_metric(self, base_context: Dict[str, Any]):
        ...
        assert "error" not in result, f"Analysis should succeed but got error: {result.get('error')}"
        ...
        assert 150 <= word_count <= 250, f"summary_text should be 150-250 words as required, got {word_count}"
    ...
    def test_full_analysis_flow_imperial(self, imperial_context: Dict[str, Any]):
        ...
        assert all(key in result for key in ["summary_text", "highlights", "confidence"])
        ...
        assert temp_class in ["hot", "warm", "cool", "cold"], f"Invalid temp classification: {temp_class}"
        assert precip_class in ["wet", "slightly wet", "dry"], f"Invalid precip classification: {precip_class}"
        assert wind_class in ["windy", "breezy", "calm"], f"Invalid wind classification: {wind_class}"
    ...
    def test_pattern_classification_accuracy(self, base_context: Dict[str, Any]):
        ...
        if total_precip >= 25 or rainy_days >= len(daily_data) * 0.5:
            assert precip_class == "wet", f"Should be wet for {total_precip}mm total, {rainy_days} rainy days"
        elif total_precip >= 5:
            assert precip_class == "slightly wet", f"Should be slightly wet for {total_precip}mm total"
        else:
    ...
    def test_summary_content_quality(self, base_context: Dict[str, Any]):
        ...
        location_mentioned = any(loc in summary.lower() for loc in [
        ...
        date_mentioned = any(term in summary.lower() for term in [
            "october", "oct", "2025", "period", "week", "days"
        ])
        assert date_mentioned, f"Summary should mention time period: {summary}"
        
        # Should contain factual temperature information
        temp_mentioned = any(term in summary for term in [
            "temperature", "°C", "°F", "warm", "hot", "cool", "cold"
        ])
        assert temp_mentioned, f"Summary should mention temperature: {summary}"
        
        # Should not contain placeholder or template text
        forbidden_phrases = ["[LOCATION]", "[DATE]", "[TEMP]", "placeholder", "template"]
        for phrase in forbidden_phrases:
            assert phrase.lower() not in summary.lower(), f"Summary contains template text: {phrase}"
...
class TestWeatherAnalystExtremes:
    ...
    def mixed_temps_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "weather_raw": {
                "location": "Test City",
                "daily": [
                    {"date": "2025-10-20", "tmin": 15.2, "tmax": 22.8, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1},
                    {"date": "2025-10-21", "tmin": 8.1, "tmax": 18.5, "precip_mm": 5.0, "wind_max_kph": 15.0, "code": 61},  # Coldest min
                    {"date": "2025-10-22", "tmin": 18.9, "tmax": 31.7, "precip_mm": 0.0, "wind_max_kph": 12.0, "code": 0},  # Hottest max
                    {"date": "2025-10-23", "tmin": 12.3, "tmax": 25.1, "precip_mm": 2.0, "wind_max_kph": 8.0, "code": 3},
                    {"date": "2025-10-24", "tmin": 16.7, "tmax": 24.9, "precip_mm": 0.0, "wind_max_kph": 11.0, "code": 2}
                ],
                "source": "open-meteo"
            }
        }
    ...
    def test_extremes_with_missing_data(self):
        ...
        """
        context_with_gaps = {
            "params": {"location": "Test", "start_date": "2025-10-20", "end_date": "2025-10-22", "units": "metric"},
            "weather_raw": {
                "daily": [
                    {"date": "2025-10-20", "tmin": None, "tmax": 25.0, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1},
                    {"date": "2025-10-21", "tmin": 12.0, "tmax": None, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1},
                    {"date": "2025-10-22", "tmin": 8.5, "tmax": 28.0, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1}
                ]
            }
        }
        ...
        if "error" in result:
            assert result["error"] == "analysis_failed", "Should return analysis_failed error"
    ...
    def test_extremes_same_day(self):
        ...
        """
        single_day_context = {
            "params": {"location": "Test", "start_date": "2025-10-20", "end_date": "2025-10-20", "units": "metric"},
            "weather_raw": {
                "daily": [
                    {"date": "2025-10-20", "tmin": 15.5, "tmax": 24.2, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1}
                ]
            }
        }
...
class TestWeatherAnalystNotableDays:
    ...
    def extreme_events_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "weather_raw": {
                "daily": [
                    {"date": "2025-10-20", "tmin": 18.0, "tmax": 25.0, "precip_mm": 0.5, "wind_max_kph": 12.0, "code": 1},     # Normal
                    {"date": "2025-10-21", "tmin": 16.0, "tmax": 23.0, "precip_mm": 15.7, "wind_max_kph": 18.0, "code": 65},   # Heavy rain (≥5mm)
                    {"date": "2025-10-22", "tmin": 14.0, "tmax": 21.0, "precip_mm": 2.0, "wind_max_kph": 48.5, "code": 3},     # Strong winds (≥40 km/h)
                    {"date": "2025-10-23", "tmin": 12.0, "tmax": 19.0, "precip_mm": 22.3, "wind_max_kph": 52.1, "code": 95},   # Thunderstorm + heavy rain + strong winds
                    {"date": "2025-10-24", "tmin": 15.0, "tmax": 22.0, "precip_mm": 0.0, "wind_max_kph": 8.0, "code": 75}      # Heavy snow
                ],
                "source": "open-meteo"
            }
        }
    ...
    def test_notable_days_heavy_rain(self, extreme_events_context: Dict[str, Any]):
        ...
        heavy_rain_days = [day for day in notable_days if "heavy rain" in day.get("note", "").lower()]
        
        # Should identify Oct 21 and Oct 23 as heavy rain days
        heavy_rain_dates = [day["date"] for day in heavy_rain_days]
        assert "2025-10-21" in heavy_rain_dates, "Oct 21 should be notable for heavy rain (15.7mm)"
        assert "2025-10-23" in heavy_rain_dates, "Oct 23 should be notable for heavy rain (22.3mm)"
        
        # Oct 20 should not be notable (only 0.5mm)
        normal_day_notes = [day["note"] for day in notable_days if day["date"] == "2025-10-20"]
        if normal_day_notes:
            assert "heavy rain" not in normal_day_notes[0].lower(), "Oct 20 should not be notable for rain"
    ...
    def test_notable_days_strong_winds(self, extreme_events_context: Dict[str, Any]):
        ...
        windy_days = [day for day in notable_days if "strong winds" in day.get("note", "").lower()]
        
        # Should identify Oct 22 and Oct 23 as windy days
        windy_dates = [day["date"] for day in windy_days]
        assert "2025-10-22" in windy_dates, "Oct 22 should be notable for strong winds (48.5 km/h)"
        assert "2025-10-23" in windy_dates, "Oct 23 should be notable for strong winds (52.1 km/h)"
    ...
    def test_notable_days_thunderstorm(self, extreme_events_context: Dict[str, Any]):
        ...
        thunderstorm_days = [day for day in notable_days if "thunderstorm" in day.get("note", "").lower()]
        thunderstorm_dates = [day["date"] for day in thunderstorm_days]
        assert "2025-10-23" in thunderstorm_dates, "Oct 23 should be notable for thunderstorm (code 95)"
        
        # Check for snow (code 75)
        snow_days = [day for day in notable_days if "snow" in day.get("note", "").lower()]
        snow_dates = [day["date"] for day in snow_days]
        assert "2025-10-24" in snow_dates, "Oct 24 should be notable for heavy snow (code 75)"
    
    def test_notable_days_multiple_criteria(self, extreme_events_context: Dict[str, Any]):
        """
        ...
        assert criteria_count >= 2, f"Oct 23 should mention multiple criteria, got: {oct_23_note}"
    ...
    def test_notable_days_empty_for_normal_weather(self):
        ...
        """
        normal_context = {
            "params": {"location": "Calm City", "start_date": "2025-10-20", "end_date": "2025-10-22", "units": "metric"},
            "weather_raw": {
                "daily": [
                    {"date": "2025-10-20", "tmin": 18.0, "tmax": 25.0, "precip_mm": 0.0, "wind_max_kph": 8.0, "code": 1},
                    {"date": "2025-10-21", "tmin": 17.0, "tmax": 24.0, "precip_mm": 1.2, "wind_max_kph": 12.0, "code": 2},
                    {"date": "2025-10-22", "tmin": 19.0, "tmax": 26.0, "precip_mm": 0.5, "wind_max_kph": 15.0, "code": 0}
                ]
            }
        }
        ...
        assert len(notable_days) <= 1, f"Normal weather should have few/no notable days, got {len(notable_days)}"
...
class TestWeatherAnalystConfidence:
    ...
    def test_confidence_range_validation(self):
        ...
        test_contexts = [
            # Complete data
            {
                "params": {"location": "Test", "start_date": "2025-10-20", "end_date": "2025-10-26", "units": "metric"},
                "weather_raw": {
                    "daily": [
                        {"date": f"2025-10-{20+i}", "tmin": 15+i, "tmax": 25+i, "precip_mm": i*2.0, "wind_max_kph": 10+i, "code": 1}
                        for i in range(7)
                    ]
                }
            },
            # Minimal data (1 day)
            {
                ...
                "weather_raw": {
                    "daily": [
                        {"date": "2025-10-20", "tmin": 15.0, "tmax": 25.0, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1}
                    ]
                }
            },
            # Moderate data (3 days)
            {
                ...
                "weather_raw": {
                    "daily": [
                        {"date": "2025-10-20", "tmin": 15.0, "tmax": 25.0, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1},
                        {"date": "2025-10-21", "tmin": 12.0, "tmax": 22.0, "precip_mm": 5.0, "wind_max_kph": 25.0, "code": 61},
                        {"date": "2025-10-22", "tmin": 18.0, "tmax": 28.0, "precip_mm": 0.0, "wind_max_kph": 8.0, "code": 0}
                    ]
                }
            }
        ]
        ...
        for i, context in enumerate(test_contexts):
            ...
            confidence = result["confidence"]
            assert isinstance(confidence, (int, float)), f"Context {i}: confidence must be numeric"
            assert 0.0 <= confidence <= 1.0, f"Context {i}: confidence {confidence} must be 0.0-1.0"
    ...
    def test_confidence_increases_with_data_quality(self):
        ...
        minimal_context = {
            ...
            "weather_raw": {
                "daily": [
                    {"date": "2025-10-20", "tmin": 20.0, "tmax": 25.0, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1}
                ]
            }
        }
        ...
        comprehensive_context = {
            ...
            "weather_raw": {
                "daily": [
                    {"date": "2025-10-20", "tmin": 10.0, "tmax": 20.0, "precip_mm": 0.0, "wind_max_kph": 8.0, "code": 1},    # Coldest
                    {"date": "2025-10-21", "tmin": 15.0, "tmax": 25.0, "precip_mm": 8.0, "wind_max_kph": 12.0, "code": 63},  # Heavy rain
                    {"date": "2025-10-22", "tmin": 18.0, "tmax": 30.0, "precip_mm": 0.0, "wind_max_kph": 45.0, "code": 3},   # Strong winds, hottest
                    {"date": "2025-10-23", "tmin": 16.0, "tmax": 24.0, "precip_mm": 2.0, "wind_max_kph": 15.0, "code": 95},  # Thunderstorm
                    {"date": "2025-10-24", "tmin": 17.0, "tmax": 26.0, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 0},
                    {"date": "2025-10-25", "tmin": 19.0, "tmax": 28.0, "precip_mm": 1.0, "wind_max_kph": 20.0, "code": 2},
                    {"date": "2025-10-26", "tmin": 18.0, "tmax": 27.0, "precip_mm": 0.0, "wind_max_kph": 11.0, "code": 1}
                ]
            }
        }
        ...
        assert comprehensive_confidence >= minimal_confidence * 0.8, \
    ...
    def test_confidence_with_partial_data(self):
        ...
        partial_context = {
            ...
            "weather_raw": {
                "daily": [
                    {"date": "2025-10-20", "tmin": None, "tmax": 25.0, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1},
                    {"date": "2025-10-21", "tmin": 15.0, "tmax": None, "precip_mm": 5.0, "wind_max_kph": 20.0, "code": 61},
                    {"date": "2025-10-22", "tmin": 18.0, "tmax": 28.0, "precip_mm": 0.0, "wind_max_kph": 8.0, "code": 0}
                ]
            }
        }
...
class TestWeatherAnalystErrorHandling:
    ...
    def test_empty_weather_raw(self):
        ...
        assert result["error"] == "no_weather_data", "Should identify no weather data error"
    ...
    def test_task_b_error_propagation(self):
        ...
        assert result == error_context, "Task B errors should be passed through unchanged"
        ...
        malformed_context = {
            ...
            "weather_raw": {
                "daily": [
                    ...
                    {"date": "2025-10-22", "tmin": "invalid", "tmax": 28.0, "precip_mm": 0.0, "wind_max_kph": 8.0, "code": 0}
                ]
            }
        }
    ...
    def test_deterministic_output(self):
        ...
        """
        context = {
            "params": {"location": "Test City", "start_date": "2025-10-20", "end_date": "2025-10-22", "units": "metric"},
            "weather_raw": {
                "daily": [
                    {"date": "2025-10-20", "tmin": 15.0, "tmax": 25.0, "precip_mm": 0.0, "wind_max_kph": 10.0, "code": 1},
                    {"date": "2025-10-21", "tmin": 12.0, "tmax": 22.0, "precip_mm": 5.0, "wind_max_kph": 25.0, "code": 61},
...
class TestWeatherAnalystIntegration:
    ...
    def test_task_b_to_task_c_integration(self):
        ...
        assert "error" not in task_c_result, f"Task C failed: {task_c_result.get('error')}"
        ...
        word_count = len(summary.split())
        assert 150 <= word_count <= 250, f"Summary should be 150-250 words as required, got {word_count} words"
    ...
    def test_real_data_analysis_quality(self):
        ...
        """
        test_locations = [
            {"location": "London", "start_date": "2025-10-20", "end_date": "2025-10-22", "units": "metric"},
            {"location": "New York", "start_date": "2025-10-20", "end_date": "2025-10-22", "units": "imperial"},
            {"location": "Sydney", "start_date": "2025-10-20", "end_date": "2025-10-22", "units": "metric"}
        ]
        ...
        for location_params in test_locations:
            ...
            if "error" not in task_b_result:
                ...
                assert "error" not in task_c_result, f"Analysis failed for {location_params['location']}"
                
                # Summary should mention the location
                summary = task_c_result["summary_text"].lower()
                location_name = location_params["location"].lower()
                assert location_name in summary, f"Summary should mention {location_name}"
                
                # Pattern should be valid
                pattern = task_c_result["highlights"]["pattern"]
                pattern_parts = pattern.split(", ")
                assert len(pattern_parts) == 3, f"Pattern should have 3 parts for {location_params['location']}"
    ...
    def test_weather_analyst_class_direct(self):
        ...
        assert class_result == function_result, "Class and function approaches should produce identical results"
